...
| 규칙 | 내용 |
|------|------|
| 입력 | 활동 **반경 UI 없음**. `intensity` 1~5는 **장소 성격만** (하루는 항상 6곳) |
| 반경 | 기본 **2.5km**. 동일 숙소 연속 투숙 시 **2.5→4→5.5…km** (cap 8) |
| 하루 구성 | 일반: 슬롯당 장소1+음식1 / 맛집: 오전·저녁 음식2, 오후 음식1+장소1 |
| 앵커 | **전날 숙소 OR 당일 숙소** 반경 안이면 후보 |
| 목적 | 카테고리 쿼터 + 주 카테고리 점수 가산 → PrimaryShare 상향 |
| 동선 | multi-recall → (LLM id 선택) → NN + hop 상한 + 슬롯 템플릿 |
