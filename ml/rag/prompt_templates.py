...
- 하루 장소 수는 항상 6곳이다. (오전·오후·저녁에 각 2곳)
- 목적이 맛집이 아니면: 음식점 3곳 + 그 외 장소 3곳
  (시간대마다 비음식 장소 1 + 음식점 1)
- 목적이 맛집이면: 음식점 5곳 + 그 외 장소 1곳
  (오전 음식점 2, 오후 음식점 1 + 비음식 1, 저녁 음식점 2)
- 활동 강도는 개수가 아니라 장소 성격이다.
  낮은 강도 → 카페·실내·완만한 산책 쪽, 높은 강도 → 언덕·산·긴 도보 쪽을
  후보 설명에 근거해 최대한 비슷하게 고른다.
...
- 일차마다 고른 장소를 나열한다. 예:
  - 장소명 (구, 카테고리) — 한 줄 이유
- 각 일차 본문 끝에 반드시:
SELECTED_IDS_DAY{{n}}: id1,id2,id3,id4,id5,id6
(후보의 id= 값만, 쉼표 구분, 그날 방문 순서대로, 정확히 6개)
"""
...
- 활동 강도: {intensity}/5 ({intensity_guide}) — 장소 성격만 맞출 것
- 탐색: {radius_text}
- 하루 장소 수: 항상 {pick_n}곳 (고정)
