...
부산을 방문하는 관광객이 **기간 / 동행 / 취향 / 숙소 / 활동 강도**를 넣으면, TourAPI 장소 데이터를 **목적 가중 multi-recall**(전날·당일 숙소 이중 앵커, 2→3→4km)로 모은 뒤 LLM이 일정을 조합하고, 동선은 nearest-neighbor(+ hop 상한)로 확정한다.
...
Streamlit  →  FastAPI  →  LangChain RAG
                              ...
                              ├─ Multi-recall (카테고리 쿼터 + geo band)

                              ├─ 숙소 앵커 (stay 확대 / 다음 숙소 가점 / 마지막 날)

                              ├─ LLM 조합 (후보 id 선택)

                              └─ NN 재정렬 + hop compact + Faithfulness 가드
...
- 숙소 좌표 필터, 활동 강도, `OPENAI_API_KEY` 연결
...
| 3차 | **활동 반경 UI 제거** · intensity→장소수/유형 · **이중 앵커 2→3→4km** · 목적 가중 상향 |



주요 모듈: `preferences.geo_band` / `stay_index_for_day`, `chain._search_weighted_hits`, `apply_next_lodging_bonus`, `enforce_max_hop`
...
- Core 지표: [`docs/EVAL.md`](EVAL.md) (`BandCompliance`, PrimaryShare 등)
